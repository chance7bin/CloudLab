 ...
 * @date 2023/04/25
 */
public interface IMSInsSocketService {

    // socket通信部分

    // 状态选择器，用于确定接收到的消息应该执行哪个步骤的方法
    void stateSelector(Channel channel, String recvMessage);

    void stateSelector(Channel channel);

    // void kill(String msg);

    void kill(Channel channel, String msg, ProcessState processState);

    void kill(Channel channel, String msg, ProcessState processState, String state, String event);
